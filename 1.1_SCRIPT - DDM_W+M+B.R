...
# STRUCTURE (cont.): $date1 (integer/date), $date2 (integer/date), $age(integer), $sex (character), and $cod (integer/character)
  ...
  pop1 <- filter(popEstimates_data_THREE_MORTALITY, YEAR == 2010)
  pop1$GENDER <- case_match(pop1$GENDER,
                            "Male" ~ "m",
                            "Female" ~ "f")
  
  pop2 <- filter(popEstimates_data_THREE_MORTALITY, YEAR == 2019)
  pop2$GENDER <- case_match(pop2$GENDER,
                            "Male" ~ "m",
                            "Female" ~ "f")
  
  DDM_data_THREE <- inner_join(pop1, pop2, by = join_by(AGE_GROUP, GENDER, RACE_COLOR)); rm(pop1, pop2)
  ...
  DDM_data_THREE <- inner_join(DDM_data_THREE, deaths, by = join_by(AGE_GROUP, GENDER, RACE_COLOR)); rm(deaths)
...
# SCRIPT: creating age groups [0,1); [1,4]; [5,9] ... for building the life table with three RACE_COLOR groups (White, Mixed, Black)
  ...
  DDM_data_THREE <- inner_join(DDM_data_THREE, find_replace, by = join_by(AGE_GROUP)); rm(find_replace)

# ---
# SCRIPT: making adjustments to the input of the ddm R function with three RACE_COLOR groups (White, Mixed, Black)
  
  DDM_data_THREE <- DDM_data_THREE %>%
    ungroup() %>% 
    rename("date1" = "YEAR.x",
           "sex" = "GENDER",
           "pop1" = "TOTAL.x",
...
# SCRIPT: running the ddm R function with three RACE_COLOR groups (White, Mixed, Black) and storing the results
  ...
  results_THREE <- ddm(DDM_data_THREE)
