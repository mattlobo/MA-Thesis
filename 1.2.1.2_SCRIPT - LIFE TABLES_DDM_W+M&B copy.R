...
# SCRIPT: preparing the input to run the life table function
  ...
  lifetable_data_THREE <- inner_join(lifetable_data_THREE,
                                   ...
                                   join_by(AGE_GROUP, GENDER, RACE_COLOR)) %>% 
    select(AGE_GROUP, LOWER, N, GENDER, RACE_COLOR, POP_1, POP_2, DEATHS, cod) # %>% 
    # mutate(DEATHS = ifelse(RACE_COLOR == "White", DEATHS * (1-0.155),
    #                       ifelse(RACE_COLOR == "Mixed", DEATHS * (1+0.08),
    #                              DEATHS * (1+0.258)))); rm(temp_data_MORTALITY_THREE)
