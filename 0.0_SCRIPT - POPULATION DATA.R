...
# SCRIPT: creating age groups [0,1); [1,4]; [5,9] ... for building the life table with all RACE_COLOR groups combined
  ...
  popEstimates_data_ALL_MORTALITY <- popEstimates_data_TWO_MORTALITY %>% 
...
# ---
# SCRIPT: creating plot to show age pyramids by race/color and gender in 2019 considering all six categories in 2019
...
# ---
# SCRIPT: creating plot to show age pyramids by race/color and gender in 2019 considering the three largest categories in 2019
  
  ggplot(data = filter(popEstimates_data_THREE_PLOT, RACE_COLOR %in% c("White", "Mixed", "Black"), YEAR == 2019),
         aes(x = AGE_GROUP, fill = RACE_COLOR, color = RACE_COLOR,
             y = ifelse(test = GENDER == "Male", yes = -TOTAL, no = TOTAL))) +
...
# SCRIPT: creating plot to show age pyramids by race/color and gender in 2019 considering the two largest categories in 2019
  ...
  ggplot(data = filter(popEstimates_data_TWO_PLOT, RACE_COLOR %in% c("White", "Mixed_Black"), YEAR == 2019),
         aes(x = AGE_GROUP, fill = RACE_COLOR, color = RACE_COLOR,
             y = ifelse(test = GENDER == "Male", yes = -TOTAL, no = TOTAL))) +
