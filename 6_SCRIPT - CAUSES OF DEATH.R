...
# SCRIPT: plotting everything
  ...
  p1 <- ggplot(rbind(filter(deathsICD10_data_WhiteMales, AGE == "Total"),
                     filter(deathsICD10_data_MixedMales, AGE == "Total"),
                     filter(deathsICD10_data_BlackMales, AGE == "Total")) %>% 
                 mutate(RACE_COLOR = ordered(RACE_COLOR, c("White", "Mixed", "Black")))) +
    ...
    guides(x = guide_axis(angle = 90, n.dodge = 1, check.overlap = TRUE)) +
  ...
  p2 <- ggplot(rbind(filter(deathsICD10_data_WhiteFemales, AGE == "Total"),
               filter(deathsICD10_data_MixedFemales, AGE == "Total"),
               filter(deathsICD10_data_BlackFemales, AGE == "Total")) %>% 
           mutate(RACE_COLOR = ordered(RACE_COLOR, c("White", "Mixed", "Black")))) +
  ...
  p3 <- ggplot(rbind(filter(deathsICD10_data_WhiteMales, AGE == "Total"),
    ...
    facet_wrap(~ RACE_COLOR, labeller = labeller(RACE_COLOR = c("White" = "White Males", "Mixed" = "Mixed Males", "Black" = "Black Males"))) +
  ...
  p4 <- ggplot(rbind(filter(deathsICD10_data_WhiteFemales, AGE == "Total"),
    ...
    facet_wrap(~ RACE_COLOR, labeller = labeller(RACE_COLOR = c("White" = "White Females", "Mixed" = "Mixed Females", "Black" = "Black Females"))) +
...
# ---
# SCRIPT: Creating the tables for the descriptive analysis
  
  tableMales <- rbind(filter(deathsICD10_data_WhiteMales, AGE == "Total"),
                      filter(deathsICD10_data_MixedMales, AGE == "Total"),
                      filter(deathsICD10_data_BlackMales, AGE == "Total")) %>% 
    select(-TOTAL) %>% 
    mutate(SHARE = round(SHARE *100, digits = 2)) %>% 
    pivot_wider(names_from = RACE_COLOR, values_from = SHARE)
  
  tableFemales <- rbind(filter(deathsICD10_data_WhiteFemales, AGE == "Total"),
                      filter(deathsICD10_data_MixedFemales, AGE == "Total"),
                      filter(deathsICD10_data_BlackFemales, AGE == "Total")) %>% 
    select(-TOTAL) %>% 
    mutate(SHARE = round(SHARE *100, digits = 2)) %>% 
